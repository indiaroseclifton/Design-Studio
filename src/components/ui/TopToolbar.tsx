  ...
  return (
    ...
    >
      ...
      {OVERLAY_BUTTONS.map((b) => (
        <button key={b.key} type="button" className={btnClass} onClick={() => showComingSoon(b.label.replace('✿ ', ''))}>
          {b.label}
        </button>
      ))}
      <span className="mx-1 h-[18px] w-px" style={{ background: 'rgba(255,240,220,.15)' }} />
      <button type="button" className={btnClass} onClick={() => showComingSoon('Add-on packs')}>
        ＋ Add-ons
      </button>
    </div>
  );
